    ...
    try {
      ...
      const response = await fetch('/api/budget/layout', {
        ...
        body: JSON.stringify({
          ...
          budget_view: budgetView,
          budget_id: budgetId,
          // Optimistic concurrency: pass the last-known updated_at so the
          // server rejects the save if another request modified the layout.
          expected_updated_at: activeLayout?.updated_at,
        }),
      });
