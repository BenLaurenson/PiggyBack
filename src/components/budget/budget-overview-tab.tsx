...
const FloatingDevTools = dynamic(
  () => import("@/components/dev/floating-dev-tools").then(mod => ({ default: mod.FloatingDevTools })),
  { ssr: false }
);
  ...
  return (
    ...
    >
      ...
      {process.env.NODE_ENV === 'development' && partnershipId && (
        <FloatingDevTools
          onClearLocalStorage={() => {
            // L163: Clear only known app-prefixed keys. Never filter by partnership
            // UUID — embedding UUIDs in localStorage keys exposes them to any
            // script on the page (including third-party).
            Object.keys(localStorage)
              .filter(key => key.startsWith('budget_') || key.startsWith('dev_tools_') || key.startsWith('piggyback-') || key.startsWith('settings_dev_tools_'))
