...
function formatPeriodDisplay(currentPeriod: Date | undefined, periodType: PeriodType, compact: boolean) {
  ...
  if (periodType === "monthly") {
    return compact
      ? currentPeriod.toLocaleDateString("en-AU", { month: "short" })
      : currentPeriod.toLocaleDateString("en-AU", { month: "long", year: "numeric" });
  }

  const month = start.toLocaleDateString("en-AU", { month: "short" });
...
export function BudgetOverviewStrip({
  ...
  const [pickerYear, setPickerYear] = useState(() => currentPeriod?.getUTCFullYear() ?? new Date().getFullYear());
  const [pickerMonth, setPickerMonth] = useState(() => currentPeriod?.getUTCMonth() ?? new Date().getMonth());
      ...
      <div className="flex items-center flex-shrink-0">
        ...
        <Popover open={calendarOpen} onOpenChange={(open) => {
            ...
            if (open && currentPeriod) {
              setPickerYear(currentPeriod.getUTCFullYear());
              setPickerMonth(currentPeriod.getUTCMonth());
            }
          }}>
              ...
              <>
                ...
                <div className="grid grid-cols-3 gap-1.5">
                  {MONTH_LABELS.map((label, i) => {
                    const isSelected =
                      currentPeriod?.getUTCFullYear() === pickerYear &&
                      currentPeriod?.getUTCMonth() === i;
