...
export function validateLayoutConfig(config: any): {
  ...
  // Validate formulas embedded in columns
  for (const col of config.columns as Column[]) {
    if (col.formula) {
      const formulaValidation = validateFormula(col.formula);
      if (!formulaValidation.valid) {
        return { valid: false, error: `Invalid formula in column "${col.name}": ${formulaValidation.error}` };
      }
    }
  }
