  ...
  return {
    searchTransactions: tool({
      ...
      execute: async ({
        ...
        if (minAmount) q = q.or(`amount_cents.lte.${-minAmount * 100},amount_cents.gte.${minAmount * 100}`);
        if (maxAmount) q = q.or(`amount_cents.gte.${-maxAmount * 100},amount_cents.lte.${maxAmount * 100}`);
...
IMPORTANT: For date ordering on transactions, use 'settled_at' or 'created_at' — there is NO 'date' or 'transaction_date' column.`,
      inputSchema: z.object({
        ...
        select: z
          .string()
          .optional()
          .describe("Columns to select (Supabase select syntax, e.g. 'name, amount_cents'). Default: '*'"),
        filters: z
          .array(
            z.object({
      ...
      execute: async ({ table, select = "*", filters = [], orderBy, limit = 100 }) => {
        const ALLOWED_TABLES = [
          ...
          "investment_history", "transaction_category_overrides",
          "tags", "transaction_tags", "net_worth_snapshots", "budget_category_shares",
        ...
        const ACCOUNT_SCOPED = ["transactions", "transaction_category_overrides", "transaction_tags"];
        ...
        if (ACCOUNT_SCOPED.includes(table)) {
          ...
          } else if (table === "transaction_category_overrides" || table === "transaction_tags") {
            // These reference transactions — scope by joining through transaction's account
            // For simplicity, no extra filter here as RLS handles it
          }
          ...
          q = q.in("id", accountIds);
        }
        // category_mappings, categories, tags are global — no scoping needed
        ...
        for (const f of filters) {
          switch (f.operator) {
            ...
            case "not.in": q = q.not(f.column, "in", `(${f.value})`); break;
          }
        }
    ...
    getSubscriptionCostTrajectory: tool({
      ...
      execute: async ({ months = 12 }) => {
        ...
        const subDetails = (expenses as Array<Record<string, unknown>>).map((exp) => {
          ...
          // Find matching transactions for price change analysis
          const merchantLower = merchant.toLowerCase();
          const merchantTxns = txns
            .filter((t) => t.description.toLowerCase().includes(merchantLower))
    ...
    createBudget: tool({
      ...
      execute: async ({ name, budgetType, emoji, methodology, periodType, budgetView, categoryFilter, totalBudget }) => {
        ...
        const slug = await generateUniqueSlug(supabase as any, partnershipId, name);
        ...
        try {
          ...
          return { error: `Budget seeding failed: ${seedErr instanceof Error ? seedErr.message : "unknown error"}` };
        }
      },
    }),
    ...
    createIncomeSource: tool({
      description:
        "Add a new income source (salary, freelance, etc.). ALWAYS confirm with the user before executing.",
      inputSchema: z.object({
        ...
        sourceType: z
          .enum(["salary", "freelance", "government", "investment", "other"])
          .optional()
          .describe("Type of income source (default 'other')"),
        nextPayDate: z.string().optional().describe("Next expected payment date in YYYY-MM-DD"),
        notes: z.string().optional(),
      }),
      inputExamples: [
        { input: { name: "Salary", amountDollars: 5000, frequency: "fortnightly", nextPayDate: "2025-02-14", sourceType: "salary" } },
      ],
      execute: async ({ name, amountDollars, frequency, sourceType, nextPayDate, notes }) => {
    ...
    getGoalDetails: tool({
      ...
      execute: async ({ goalName, includeContributions = false }) => {
        ...
        if (goalName) {
          goalQuery = goalQuery.ilike("name", `%${goalName}%`);
        }
