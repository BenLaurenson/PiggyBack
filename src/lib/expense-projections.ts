...
export function hasPaymentInPeriod(
  ...
  return matches.some(match => {
    ...
    if (txn && (txn.created_at || txn.settled_at)) {
      const txnDate = new Date(txn.created_at || txn.settled_at);
...
export function generatePaidInstances(
  ...
  expenses.forEach(expense => {
    ...
    const periodMatches = matches.filter(match => {
      ...
      const txn = Array.isArray(rawTxn) ? rawTxn[0] : rawTxn;
      if (txn && (txn.created_at || txn.settled_at)) {
        const dateStr = txn.created_at || txn.settled_at;
    ...
    periodMatches.forEach((match, index) => {
      ...
      const paymentDate = txn?.created_at || txn?.settled_at || match.matched_at;
