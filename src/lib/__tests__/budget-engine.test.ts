...
describe("getBudgetPeriodRange", () => {
  describe("monthly", () => {
    ...
    it("handles leap year February", () => {
      const result = getBudgetPeriodRange(new Date("2028-02-15"), "monthly");
      expect(result.end).toEqual(
    ...
    it("handles 31-day months", () => {
      const result = getBudgetPeriodRange(new Date("2026-01-20"), "monthly");
      expect(result.end).toEqual(
  ...
  describe("weekly (month-aligned: 1-7, 8-14, 15-21, 22-end)", () => {
    it("week 1: day 1-7", () => {
    ...
    it("week 2: day 8-14", () => {
      const result = getBudgetPeriodRange(new Date("2026-02-10"), "weekly");
      expect(result.start).toEqual(
    ...
    it("week 3: day 15-21", () => {
      const result = getBudgetPeriodRange(new Date("2026-02-18"), "weekly");
      expect(result.start).toEqual(
    ...
    it("week 4: day 22 to end of month", () => {
      const result = getBudgetPeriodRange(new Date("2026-02-25"), "weekly");
      expect(result.start).toEqual(
    ...
    it("week 4 of 31-day month extends to 31st", () => {
      const result = getBudgetPeriodRange(new Date("2026-01-30"), "weekly");
      expect(result.end).toEqual(
        new Date(Date.UTC(2026, 0, 31, 23, 59, 59, 999))
      );
    });
  ...
  describe("fortnightly (month-aligned: 1-14, 15-end)", () => {
...
describe("period navigation", () => {
  describe("getNextPeriodDate", () => {
    it("monthly: advances to next month", () => {
      const result = getNextPeriodDate(new Date("2026-02-15"), "monthly");
      expect(result.getUTCMonth()).toBe(2); // March
      expect(result.getUTCDate()).toBe(1);
    });

    it("weekly: advances to next week boundary", () => {
      const result = getNextPeriodDate(new Date("2026-02-03"), "weekly");
      expect(result.getUTCDate()).toBe(8);
    });

    it("weekly: week 4 wraps to next month week 1", () => {
      const result = getNextPeriodDate(new Date("2026-02-25"), "weekly");
      expect(result.getUTCMonth()).toBe(2); // March
      expect(result.getUTCDate()).toBe(1);
    });

    it("fortnightly: first half to second half", () => {
      const result = getNextPeriodDate(new Date("2026-02-08"), "fortnightly");
      expect(result.getUTCDate()).toBe(15);
    });

    it("fortnightly: second half wraps to next month", () => {
      const result = getNextPeriodDate(new Date("2026-02-20"), "fortnightly");
      expect(result.getUTCMonth()).toBe(2); // March
      expect(result.getUTCDate()).toBe(1);
    });
  });
  ...
  describe("getPreviousPeriodDate", () => {
    it("monthly: goes to previous month", () => {
      const result = getPreviousPeriodDate(new Date("2026-02-15"), "monthly");
      expect(result.getUTCMonth()).toBe(0); // January
    });

    it("weekly: week 2 goes to week 1", () => {
      const result = getPreviousPeriodDate(new Date("2026-02-10"), "weekly");
      expect(result.getUTCDate()).toBe(1);
    });

    it("weekly: week 1 goes to previous month week 4", () => {
      const result = getPreviousPeriodDate(new Date("2026-02-03"), "weekly");
      expect(result.getUTCMonth()).toBe(0); // January
  ...
  describe("getMonthKeyForPeriod", () => {
    it("returns ISO month key for period date", () => {
      expect(getMonthKeyForPeriod(new Date("2026-02-15"))).toBe("2026-02-01");
    });
  });
});
