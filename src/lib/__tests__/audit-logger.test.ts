...
describe('audit-logger', () => {
  ...
  describe('AuditAction', () => {
    ...
    it('should define all security audit actions', async () => {
      const { AuditAction } = await import('@/lib/audit-logger');

      expect(AuditAction.PASSWORD_CHANGED).toBe('PASSWORD_CHANGED');
      expect(AuditAction.ACCOUNT_DELETED).toBe('ACCOUNT_DELETED');
      expect(AuditAction.PARTNER_ADDED).toBe('PARTNER_ADDED');
      expect(AuditAction.PARTNER_REMOVED).toBe('PARTNER_REMOVED');
      expect(AuditAction.FINANCIAL_DATA_EXPORTED).toBe('FINANCIAL_DATA_EXPORTED');
    });

    it('should have at least 10 defined actions', async () => {
      const { AuditAction } = await import('@/lib/audit-logger');
