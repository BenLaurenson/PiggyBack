  ...
  describe("single transaction recategorize", () => {
    it("should create override and update transaction", async () => {
      ...
      tableChains["accounts"] = createChain({
        data: { id: "acc-1", user_id: "user-123" },
        error: null,
      });
      ...
      const request = createRequest("txn-1", {
    ...
    it("should return 401 when not authenticated", async () => {
      ...
      const request = createRequest("txn-1", { category_id: "groceries" });
      const response = await PATCH(request, {
        params: Promise.resolve({ id: "txn-1" }),
      });
    ...
    it("should return 403 when user does not own the transaction", async () => {
      ...
      tableChains["transactions"] = createChain({
        data: { id: "txn-1", category_id: "groceries", parent_category_id: "home", account_id: "acc-1", description: "Test" },
        error: null,
      });

      // Different user owns the account
      tableChains["accounts"] = createChain({
        data: { id: "acc-1", user_id: "other-user" },
        error: null,
      });
  ...
  describe("DELETE — reset to original", () => {
    it("should restore original categories and delete override", async () => {
      ...
      const request = new Request(
        "http://localhost:3000/api/transactions/txn-1/recategorize",
        { method: "DELETE" }
      );
