      ...
      if (linkedGoals && linkedGoals.length > 0) {
        for (const goal of linkedGoals) {
          if (goal.current_amount_cents !== newBalance) {
            ...
            // Record contribution history for savings progress chart
            await supabase
 ...
 */
async function processTransactionDeletion(upTransactionId: string) {
  const supabase = createServiceRoleClient();
  ...
  // 2. Remove associated expense matches
  ...
  // 3. Soft-delete the transaction (mark as deleted)
  ...
  try {
    ...
    switch (eventType) {
      ...
      case "TRANSACTION_DELETED": {
        ...
        if (deletedTxnId) {
          await processTransactionDeletion(deletedTxnId);
        }
        break;
      }
