...
export async function POST(request: Request) {
  ...
  const customCategorySchema = z.object({
    ...
    underlyingCategories: z.array(z.string().min(1).max(100)),
  ...
  const customizeSchema = z.object({
    ...
    methodology: z.enum(['zero-based', '50-30-20', 'envelope', 'pay-yourself-first', '80-20']),
    custom_categories: z.array(customCategorySchema).max(50).optional().default([]),
    hidden_subcategories: z.array(z.string().min(1).max(200)).max(100).optional().default([]),
    is_partnership_wide: z.boolean().optional().default(false),
    expected_updated_at: z.string().optional(),
  });

  const parsed = await parseBody(request, customizeSchema);
  ...
  try {
    // Use RPC for atomic upsert with optimistic concurrency.
    // If expected_updated_at is provided, the function only updates if the row
    // hasn't been modified since the caller last read it — preventing race
    // conditions where two concurrent saves overwrite each other's JSONB changes.
    const { data: rows, error: rpcError } = await supabase.rpc(
      "update_methodology_customization",
      {
        p_partnership_id: partnership_id,
        p_user_id: is_partnership_wide ? null : user.id,
        p_methodology_name: methodology,
        p_custom_categories: custom_categories || [],
        p_hidden_subcategories: hidden_subcategories || [],
        p_expected_updated_at: expected_updated_at || null,
      }
    );

    if (rpcError) {
      console.error("Failed to save customization:", rpcError);
      return NextResponse.json(
    ...
    const customization = Array.isArray(rows) ? rows[0] : rows;

    // If expected_updated_at was provided but no rows were returned,
    // the row was modified by another request (optimistic concurrency conflict).
    if (!customization && expected_updated_at) {
      return NextResponse.json(
        { error: "Customization was modified by another request. Please refresh and try again." },
        { status: 409 }
      );
    }
