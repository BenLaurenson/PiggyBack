...
// Cron endpoint: 5 requests per minute (generous for retries, but prevents abuse)
...
export async function GET(req: Request) {
  // Rate limit by IP to prevent brute-force attempts on CRON_SECRET
  ...
  // --- Weekly Summaries ---
  try {
    // H13 fix: Don't bulk-fetch ai_api_key for all users. Fetch keys one at a
    // time only for users that pass all eligibility checks, so decrypted keys
    // are never held in memory en masse.
    const { data: summaryUsers } = await supabase
      .from("profiles")
      .select("id, notification_preferences, ai_provider, ai_model")
      .limit(500);
    ...
    for (const profile of summaryUsers || []) {
      try {
        ...
        // Fetch AI key only for this user, only after all eligibility checks pass
        ...
        if (!aiProfile?.ai_api_key) continue;

        // Set up AI model
        const provider = profile.ai_provider || "google";
        const apiKey = getPlaintextToken(aiProfile.ai_api_key);
        let baseModel;
