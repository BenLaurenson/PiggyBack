...
export async function createExpenseFromTransaction(
  ...
  try {
    ...
    if (expenseError) {
      ...
      throw new Error(`Database error: ${expenseError.message || expenseError.code || JSON.stringify(expenseError)}`);
    }
    ...
    console.error('createExpenseFromTransaction error:', error);
 ...
 * Cascades to expense_matches + couple_split_settings via FK.
 * Also cleans up orphaned transaction_references.
 */
export async function deleteExpense(
  ...
  try {
    ...
    // Clean up transaction_references (no FK cascade on this table)
    await supabase
      .from("transaction_references")
      .delete()
      .eq("reference_type", "expense_definition")
      .eq("reference_id", expenseId);

    // Delete expense definition (cascades to expense_matches + couple_split_settings)
...
export async function getVendorTransactionHistory(
  ...
  try {
    ...
    const { data: transactions, error } = await supabase
      ...
      .ilike('description', `%${vendorPattern}%`)
    ...
    console.error('getVendorTransactionHistory error:', error);
    return {
      transactions: [],
