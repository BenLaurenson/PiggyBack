...
export async function registerUpWebhook(): Promise<{
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const errorMessage = errorData.errors?.[0]?.detail || "Failed to register webhook with Up Bank";
    ...
    const { error: updateError } = await supabase
      ...
      .update({
        ...
        webhook_secret: result.data.attributes.secretKey,
    ...
    console.error("registerUpWebhook error:", error);
    return {
...
export async function deleteUpWebhook(): Promise<{
  ...
  try {
    ...
    console.error("deleteUpWebhook error:", error);
    return {
...
export async function pingWebhook(): Promise<{
  ...
  try {
    ...
    console.error("pingWebhook error:", error);
    return {
      error: error instanceof Error ? error.message : "Failed to ping webhook"
    };
  }
}
...
export async function connectUpBank(plaintextToken: string): Promise<{
  ...
  try {
    ...
    // 2. Encrypt the token before storage
    let tokenToStore = plaintextToken;
    if (process.env.UP_API_ENCRYPTION_KEY) {
      tokenToStore = encryptToken(plaintextToken);
    }
    ...
    console.error("connectUpBank error:", error);
    return {
