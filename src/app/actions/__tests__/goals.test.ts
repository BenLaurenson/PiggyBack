...
vi.mock("@/lib/get-user-partnership", () => ({
  getUserPartnershipId: vi.fn(() => Promise.resolve("partnership-1")),
}));

function createGoalsMockSupabase(goalData: Record<string, unknown> | null = null) {
  const updateChain: Record<string, ReturnType<typeof vi.fn>> & { then?: any } = {};
  updateChain.eq = vi.fn(() => updateChain);
  updateChain.update = vi.fn(() => updateChain);
  // Make the chain thenable so `await supabase.from(...).update(...).eq(...).eq(...)` resolves
  ...
  // Mock supabase.rpc for atomic add_funds_to_goal
  const rpcMock = vi.fn((_fnName: string, params: Record<string, unknown>) => {
    const currentAmount = (goalData?.current_amount_cents as number) ?? 0;
    const addedAmount = (params?.p_amount_cents as number) ?? 0;
    const newAmount = currentAmount + addedAmount;
    const targetAmount = (goalData?.target_amount_cents as number) ?? 0;
    return Promise.resolve({
      data: {
        new_amount_cents: newAmount,
        is_completed: newAmount >= targetAmount,
        completed_at: newAmount >= targetAmount ? new Date().toISOString() : null,
      },
      error: null,
    });
  });
