...
const incomeModeSchema = z.enum(['recurring', 'one-off']);
const oneOffTypeSchema = z.enum(['bonus', 'gift', 'dividend', 'tax-refund', 'freelance', 'other']);

const createIncomeOptionsSchema = z.object({
  customName: z.string().max(200).optional(),
  oneOffType: oneOffTypeSchema.optional(),
});
...
export async function syncIncomeTagsFromUpBank(): Promise<{
  ...
  if (blocked) return { synced: 0, error: blocked.error };
  ...
  try {
    ...
    return {
      synced: 0,
      error: safeErrorMessage(error, 'Failed to sync income tags'),
    };
  }
}
...
export async function createIncomeSourceFromTransaction(
  ...
  const modeParsed = incomeModeSchema.safeParse(mode);
  if (!modeParsed.success) return { success: false, error: 'Invalid mode' };
  const optsParsed = createIncomeOptionsSchema.safeParse(options);
  ...
  try {
    ...
    if (flagError) {
      throw new Error('Failed to update transaction flags');
    }
    ...
    return {
      success: false,
      error: safeErrorMessage(error, 'Failed to create income source'),
    };
  }
}
