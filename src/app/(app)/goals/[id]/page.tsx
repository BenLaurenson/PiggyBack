...
export default async function GoalDetailPage({ params, searchParams }: GoalDetailPageProps) {
  ...
  // Parallel data fetching — keep this set wide enough to power both
  // the existing chart/log and the new payday/projection cards.
  const [
    { data: contributions },
    { data: goalAssignment },
    { data: incomeSources },
    ...
    supabase
      ...
      .maybeSingle(),
    // Configured recurring income — preferred source for "next payday".
    supabase
      .from("income_sources")
      .select("frequency, next_pay_date, source_type, is_active")
      .eq("partnership_id", goal.partnership_id),
    // Salary-tagged transactions for fallback pattern detection. We
    // accept anything flagged is_income=true with income_type 'salary'
    // — see src/app/actions/income.ts for how those get tagged.
    user?.id
      ? supabase
          .from("transactions")
    ...
    // The current user's accounts — used to gate the income query above.
    user?.id
