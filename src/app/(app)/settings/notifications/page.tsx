...
export default function NotificationSettingsPage() {
  ...
  const loadPreferences = async () => {
    ...
    const { data: profile } = await supabase
      .from("profiles")
      .select("notification_preferences, ai_provider")
      .eq("id", user.id)
      .maybeSingle();

    // Check if AI is configured by looking at ai_provider (avoids fetching the actual key)
    setHasAiKey(!!profile?.ai_provider);
  ...
  const handleSave = async () => {
    ...
    try {
      const result = await updateNotificationPreferences(prefs as unknown as Record<string, unknown>);
