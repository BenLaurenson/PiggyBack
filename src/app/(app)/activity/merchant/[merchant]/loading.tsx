import { Skeleton } from "@/components/ui/skeleton";
import { Card, CardContent, CardHeader } from "@/components/ui/card";

export default function MerchantActivityLoading() {
  return (
    <div
      className="p-4 md:p-6 space-y-6"
      style={{ backgroundColor: "var(--background)" }}
    >
      {/* Back Button */}
      <div className="flex items-center gap-1 mb-2">
        <Skeleton className="h-4 w-4 rounded" />
        <Skeleton className="h-4 w-24" />
      </div>

      {/* Hero Section (merchant has no icon, just name + subtitle) */}
      <div className="space-y-1">
        <Skeleton className="h-8 sm:h-9 md:h-10 w-48" />
        <Skeleton className="h-4 sm:h-5 w-44" />
      </div>

      {/* Stats Grid - 2x2 on mobile, 4 cols on desktop */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {/* Total Spent */}
        <Card
          className="border-0 shadow-lg overflow-hidden"
          style={{ backgroundColor: "var(--pastel-coral-light)" }}
        >
          <CardContent className="p-4">
            <Skeleton className="h-7 sm:h-8 w-24 mb-1" />
            <Skeleton className="h-3 w-16" />
          </CardContent>
        </Card>

        {/* Per Transaction */}
        <Card
          className="border-0 shadow-lg"
          style={{ backgroundColor: "var(--surface-elevated)" }}
        >
          <CardContent className="p-4">
            <Skeleton className="h-7 sm:h-8 w-20 mb-1" />
            <Skeleton className="h-3 w-24" />
          </CardContent>
        </Card>

        {/* Transactions */}
        <Card
          className="border-0 shadow-lg overflow-hidden"
          style={{ backgroundColor: "var(--pastel-blue-light)" }}
        >
          <CardContent className="p-4">
            <Skeleton className="h-7 sm:h-8 w-12 mb-1" />
            <Skeleton className="h-3 w-20" />
          </CardContent>
        </Card>

        {/* Since */}
        <Card
          className="border-0 shadow-lg overflow-hidden"
          style={{ backgroundColor: "var(--pastel-yellow-light)" }}
        >
          <CardContent className="p-4">
            <Skeleton className="h-7 sm:h-8 w-14 mb-1" />
            <Skeleton className="h-3 w-10" />
          </CardContent>
        </Card>
      </div>

      {/* Trend Chart with filter pills inside */}
      <Card
        className="border-0 shadow-lg"
        style={{ backgroundColor: "var(--surface-elevated)" }}
      >
        <CardHeader className="pb-2">
          <div className="space-y-3">
            <Skeleton className="h-5 w-36" />
            <div className="flex gap-2 flex-wrap">
              <Skeleton className="h-7 w-20 rounded-full" />
              <Skeleton className="h-7 w-20 rounded-full" />
              <Skeleton className="h-7 w-20 rounded-full" />
            </div>
          </div>
        </CardHeader>
        <CardContent>
          <Skeleton className="h-[200px] w-full rounded-lg" />
        </CardContent>
      </Card>

      {/* Transactions List - simple divide-y, no date group headers */}
      <Card
        className="border-0 shadow-lg"
        style={{ backgroundColor: "var(--surface-elevated)" }}
      >
        <CardHeader className="flex flex-row items-center justify-between pb-2">
          <Skeleton className="h-5 w-36" />
        </CardHeader>
        <CardContent>
          <div
            className="divide-y"
            style={{ borderColor: "var(--border)" }}
          >
            {Array.from({ length: 8 }).map((_, i) => (
              <div
                key={i}
                className="py-3 flex items-center justify-between gap-2"
              >
                <div className="flex-1 min-w-0 space-y-1">
                  <Skeleton className="h-4 w-24" />
                  <Skeleton className="h-3 w-20" />
                </div>
                <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
                  <Skeleton className="h-5 w-16 rounded-full hidden sm:block" />
                  <Skeleton className="h-4 w-16" />
                </div>
              </div>
            ))}
          </div>
        </CardContent>
      </Card>
    </div>
  );
}
