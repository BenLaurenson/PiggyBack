...
export default async function BudgetPage({
  ...
  if (!params.id) {
    ...
    // Compute lightweight stats per budget using standalone aggregation functions.
    // Previously this ran the full calculateBudgetSummary() engine per budget
    // (N full engine runs with row-building pipeline). Now we use calculateBudgeted()
    // and calculateSpent() directly, which only compute totals without building rows.
    ...
    for (const budget of userBudgets) {
      ...
      // calculateBudgeted: sums assignments + expense defaults (no row building)
      const totalBudgeted = calculateBudgeted(
        assignments,
        sharedExpenseDefs,
        sharedSplits,
        budget.period_type,
      ...
      // calculateSpent: maps transactions to categories and sums (no row building)
      ...
      // Category count: distinct categories with a positive assignment
      const categoryCount = new Set(
        ba.filter(a => a.assigned_cents > 0).map(a => a.category_name)
      ).size;
