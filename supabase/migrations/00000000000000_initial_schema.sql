...
-- This single file replaces 53 individual migration files (001-051)
...
BEGIN
  IF NEW.category_id IS DISTINCT FROM OLD.category_id OR
    ...
    DELETE FROM expense_matches WHERE transaction_id = NEW.id;
  END IF;
  RETURN NEW;
END;
$function$;
...
CREATE OR REPLACE FUNCTION public.merge_partnerships(p_primary_partnership_id uuid, p_joining_user_id uuid, p_joining_partnership_id uuid, p_link_request_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
AS $function$
BEGIN
  UPDATE public.income_sources SET partnership_id = p_primary_partnership_id
    WHERE partnership_id = p_joining_partnership_id;
  UPDATE public.expense_definitions SET partnership_id = p_primary_partnership_id
    WHERE partnership_id = p_joining_partnership_id;
  UPDATE public.savings_goals SET partnership_id = p_primary_partnership_id
    WHERE partnership_id = p_joining_partnership_id;
  UPDATE public.investments SET partnership_id = p_primary_partnership_id
    WHERE partnership_id = p_joining_partnership_id;

  DELETE FROM public.budget_assignments ba
    WHERE ba.partnership_id = p_joining_partnership_id
    AND EXISTS (SELECT 1 FROM public.budget_assignments ba2
      WHERE ba2.partnership_id = p_primary_partnership_id
      AND ba2.month = ba.month AND ba2.category_name = ba.category_name);
  UPDATE public.budget_assignments SET partnership_id = p_primary_partnership_id
    WHERE partnership_id = p_joining_partnership_id;

  DELETE FROM public.couple_split_settings css
    WHERE css.partnership_id = p_joining_partnership_id
    AND EXISTS (SELECT 1 FROM public.couple_split_settings css2
      WHERE css2.partnership_id = p_primary_partnership_id
      AND css2.category_name IS NOT DISTINCT FROM css.category_name
      AND css2.expense_definition_id IS NOT DISTINCT FROM css.expense_definition_id);
  UPDATE public.couple_split_settings SET partnership_id = p_primary_partnership_id
    WHERE partnership_id = p_joining_partnership_id;

  DELETE FROM public.budget_months bm
    WHERE bm.partnership_id = p_joining_partnership_id
    AND EXISTS (SELECT 1 FROM public.budget_months bm2
      WHERE bm2.partnership_id = p_primary_partnership_id AND bm2.month = bm.month);
  UPDATE public.budget_months SET partnership_id = p_primary_partnership_id
    WHERE partnership_id = p_joining_partnership_id;

  INSERT INTO public.partnership_members (partnership_id, user_id, role)
    VALUES (p_primary_partnership_id, p_joining_user_id, 'member')
    ON CONFLICT (partnership_id, user_id) DO NOTHING;
  DELETE FROM public.partnership_members
    WHERE user_id = p_joining_user_id AND partnership_id = p_joining_partnership_id;

  DELETE FROM public.partnerships WHERE id = p_joining_partnership_id
    AND NOT EXISTS (SELECT 1 FROM public.partnership_members WHERE partnership_id = p_joining_partnership_id);

  UPDATE public.partner_link_requests
    SET status = 'accepted', primary_partnership_id = p_primary_partnership_id, updated_at = NOW()
    WHERE id = p_link_request_id;
  UPDATE public.partnerships SET name = 'Our Budget', updated_at = NOW()
    WHERE id = p_primary_partnership_id;

  RETURN jsonb_build_object('success', true, 'primary_partnership_id', p_primary_partnership_id);
END; $function$;
...
CREATE TABLE public.profiles (
  ...
  alpha_vantage_calls_today integer DEFAULT 0,
  alpha_vantage_calls_date date,
...
CREATE INDEX idx_budget_months_partnership_month ON public.budget_months USING btree (partnership_id, month);

-- budgets
CREATE INDEX idx_budgets_partnership_id ON public.budgets USING btree (partnership_id);

-- category_mappings
CREATE INDEX idx_category_mappings_up_id ON public.category_mappings USING btree (up_category_id);

-- category_pin_states
CREATE INDEX idx_category_pin_states_user ON public.category_pin_states USING btree (user_id, methodology_name);

-- budget_category_shares
CREATE INDEX idx_category_shares_category ON public.budget_category_shares USING btree (partnership_id, category_name);
CREATE INDEX idx_category_shares_partnership ON public.budget_category_shares USING btree (partnership_id);

-- couple_split_settings
CREATE INDEX idx_couple_split_settings_partnership ON public.couple_split_settings USING btree (partnership_id);
...
-- user_budgets
CREATE INDEX idx_user_budgets_default ON public.user_budgets USING btree (partnership_id) WHERE (is_default = true);
...
-- ============================================================================
-- 10. RLS POLICIES (batch 1: a-i tables)
-- ============================================================================

-- accounts
CREATE POLICY "Partners can view each others accounts" ON public.accounts AS PERMISSIVE FOR SELECT TO public USING ((user_id IN ( SELECT get_partner_user_ids.user_id FROM private.get_partner_user_ids(( SELECT auth.uid() AS uid)) get_partner_user_ids(user_id))));
CREATE POLICY "Users can insert own accounts" ON public.accounts AS PERMISSIVE FOR INSERT TO public WITH CHECK ((user_id = auth.uid()));
CREATE POLICY "Users can update own accounts" ON public.accounts AS PERMISSIVE FOR UPDATE TO public USING ((user_id = auth.uid()));
CREATE POLICY "Users can view own accounts" ON public.accounts AS PERMISSIVE FOR SELECT TO public USING ((user_id = auth.uid()));

-- annual_checkups
CREATE POLICY "Members can create partnership checkups" ON public.annual_checkups AS PERMISSIVE FOR INSERT TO public WITH CHECK ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Members can delete partnership checkups" ON public.annual_checkups AS PERMISSIVE FOR DELETE TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Members can update partnership checkups" ON public.annual_checkups AS PERMISSIVE FOR UPDATE TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Members can view partnership checkups" ON public.annual_checkups AS PERMISSIVE FOR SELECT TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));

-- budget_assignments
CREATE POLICY "Members can create partnership budget assignments" ON public.budget_assignments AS PERMISSIVE FOR INSERT TO public WITH CHECK ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Members can delete partnership budget assignments" ON public.budget_assignments AS PERMISSIVE FOR DELETE TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Members can update partnership budget assignments" ON public.budget_assignments AS PERMISSIVE FOR UPDATE TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Members can view partnership budget assignments" ON public.budget_assignments AS PERMISSIVE FOR SELECT TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));

-- budget_category_shares
CREATE POLICY "Users can delete their partnership's category shares" ON public.budget_category_shares AS PERMISSIVE FOR DELETE TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Users can insert category shares for their partnership" ON public.budget_category_shares AS PERMISSIVE FOR INSERT TO public WITH CHECK ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Users can update their partnership's category shares" ON public.budget_category_shares AS PERMISSIVE FOR UPDATE TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Users can view their partnership's category shares" ON public.budget_category_shares AS PERMISSIVE FOR SELECT TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));

-- budget_item_preferences
CREATE POLICY "Users can create their own category preferences" ON public.budget_item_preferences AS PERMISSIVE FOR INSERT TO public WITH CHECK ((user_id = auth.uid()));
CREATE POLICY "Users can delete their own category preferences" ON public.budget_item_preferences AS PERMISSIVE FOR DELETE TO public USING ((user_id = auth.uid()));
CREATE POLICY "Users can update their own category preferences" ON public.budget_item_preferences AS PERMISSIVE FOR UPDATE TO public USING ((user_id = auth.uid()));
CREATE POLICY "Users can view their own category preferences" ON public.budget_item_preferences AS PERMISSIVE FOR SELECT TO public USING ((user_id = auth.uid()));

-- budget_months
CREATE POLICY "Members can create partnership budget months" ON public.budget_months AS PERMISSIVE FOR INSERT TO public WITH CHECK ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Members can update partnership budget months" ON public.budget_months AS PERMISSIVE FOR UPDATE TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Members can view partnership budget months" ON public.budget_months AS PERMISSIVE FOR SELECT TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));

-- budgets
CREATE POLICY "Members can create partnership budgets" ON public.budgets AS PERMISSIVE FOR INSERT TO public WITH CHECK ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Members can update partnership budgets" ON public.budgets AS PERMISSIVE FOR UPDATE TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Members can view partnership budgets" ON public.budgets AS PERMISSIVE FOR SELECT TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));

-- categories
CREATE POLICY "Anyone can insert categories" ON public.categories AS PERMISSIVE FOR INSERT TO public WITH CHECK (true);
CREATE POLICY "Anyone can view categories" ON public.categories AS PERMISSIVE FOR SELECT TO public USING (true);
CREATE POLICY "Authenticated users can insert categories" ON public.categories AS PERMISSIVE FOR INSERT TO public WITH CHECK ((auth.uid() IS NOT NULL));
CREATE POLICY "Authenticated users can update categories" ON public.categories AS PERMISSIVE FOR UPDATE TO public USING ((auth.uid() IS NOT NULL));

-- category_mappings
CREATE POLICY "Category mappings are viewable by all authenticated users" ON public.category_mappings AS PERMISSIVE FOR SELECT TO public USING (true);

-- category_pin_states
CREATE POLICY "Users can manage their own pin states" ON public.category_pin_states AS PERMISSIVE FOR ALL TO public USING ((user_id = auth.uid()));

-- couple_split_settings
CREATE POLICY "Members can create partnership split settings" ON public.couple_split_settings AS PERMISSIVE FOR INSERT TO public WITH CHECK ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Members can delete partnership split settings" ON public.couple_split_settings AS PERMISSIVE FOR DELETE TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Members can update partnership split settings" ON public.couple_split_settings AS PERMISSIVE FOR UPDATE TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Members can view partnership split settings" ON public.couple_split_settings AS PERMISSIVE FOR SELECT TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));

-- expense_definitions
CREATE POLICY "Members can create partnership expenses" ON public.expense_definitions AS PERMISSIVE FOR INSERT TO public WITH CHECK ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Members can delete partnership expenses" ON public.expense_definitions AS PERMISSIVE FOR DELETE TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Members can update partnership expenses" ON public.expense_definitions AS PERMISSIVE FOR UPDATE TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Members can view partnership expenses" ON public.expense_definitions AS PERMISSIVE FOR SELECT TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));

-- expense_matches
CREATE POLICY "Members can create expense matches" ON public.expense_matches AS PERMISSIVE FOR INSERT TO public WITH CHECK ((expense_definition_id IN ( SELECT expense_definitions.id FROM expense_definitions WHERE (expense_definitions.partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))))));
CREATE POLICY "Members can delete expense matches" ON public.expense_matches AS PERMISSIVE FOR DELETE TO public USING ((expense_definition_id IN ( SELECT expense_definitions.id FROM expense_definitions WHERE (expense_definitions.partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))))));
CREATE POLICY "Members can view expense matches" ON public.expense_matches AS PERMISSIVE FOR SELECT TO public USING ((expense_definition_id IN ( SELECT expense_definitions.id FROM expense_definitions WHERE (expense_definitions.partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))))));

-- income_sources
CREATE POLICY "Partners can delete income sources" ON public.income_sources AS PERMISSIVE FOR DELETE TO public USING ((user_id IN ( SELECT pm.user_id FROM partnership_members pm WHERE (pm.partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))))));
CREATE POLICY "Partners can insert income for each other" ON public.income_sources AS PERMISSIVE FOR INSERT TO public WITH CHECK ((user_id IN ( SELECT pm.user_id FROM partnership_members pm WHERE (pm.partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))))));
CREATE POLICY "Partners can update income sources" ON public.income_sources AS PERMISSIVE FOR UPDATE TO public USING ((user_id IN ( SELECT pm.user_id FROM partnership_members pm WHERE (pm.partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))))));
CREATE POLICY "Users can insert own income sources" ON public.income_sources AS PERMISSIVE FOR INSERT TO public WITH CHECK ((auth.uid() = user_id));
CREATE POLICY "Users can view own income sources" ON public.income_sources AS PERMISSIVE FOR SELECT TO public USING ((auth.uid() = user_id));
CREATE POLICY "Users can view partnership income sources" ON public.income_sources AS PERMISSIVE FOR SELECT TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));

-- investment_history
CREATE POLICY "Members can insert investment history" ON public.investment_history AS PERMISSIVE FOR INSERT TO public WITH CHECK ((investment_id IN ( SELECT investments.id FROM investments WHERE (investments.partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))))));
CREATE POLICY "Members can view investment history" ON public.investment_history AS PERMISSIVE FOR SELECT TO public USING ((investment_id IN ( SELECT investments.id FROM investments WHERE (investments.partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))))));

-- investment_contributions
CREATE POLICY "Members can view investment contributions" ON public.investment_contributions AS PERMISSIVE FOR SELECT TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Members can insert investment contributions" ON public.investment_contributions AS PERMISSIVE FOR INSERT TO public WITH CHECK ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Members can delete investment contributions" ON public.investment_contributions AS PERMISSIVE FOR DELETE TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));

-- investments
CREATE POLICY "Members can create partnership investments" ON public.investments AS PERMISSIVE FOR INSERT TO public WITH CHECK ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Members can delete partnership investments" ON public.investments AS PERMISSIVE FOR DELETE TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Members can update partnership investments" ON public.investments AS PERMISSIVE FOR UPDATE TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Members can view partnership investments" ON public.investments AS PERMISSIVE FOR SELECT TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));

-- ============================================================================
-- 11. RLS POLICIES (batch 2: j-z tables)
-- ============================================================================

-- merchant_category_rules
CREATE POLICY "Users can create their own merchant rules" ON public.merchant_category_rules AS PERMISSIVE FOR INSERT TO public WITH CHECK ((user_id = auth.uid()));
CREATE POLICY "Users can delete their own merchant rules" ON public.merchant_category_rules AS PERMISSIVE FOR DELETE TO public USING ((user_id = auth.uid()));
CREATE POLICY "Users can update their own merchant rules" ON public.merchant_category_rules AS PERMISSIVE FOR UPDATE TO public USING ((user_id = auth.uid()));
CREATE POLICY "Users can view their own merchant rules" ON public.merchant_category_rules AS PERMISSIVE FOR SELECT TO public USING ((user_id = auth.uid()));

-- methodology_customizations
CREATE POLICY "Members can manage their own customizations" ON public.methodology_customizations AS PERMISSIVE FOR ALL TO public USING (((user_id = auth.uid()) OR ((user_id IS NULL) AND (partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE ((partnership_members.user_id = auth.uid()) AND (partnership_members.role = 'owner'::text)))))));
CREATE POLICY "Members can view partnership customizations" ON public.methodology_customizations AS PERMISSIVE FOR SELECT TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));

-- milestones
CREATE POLICY "Members can create partnership milestones" ON public.milestones AS PERMISSIVE FOR INSERT TO public WITH CHECK ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Members can delete partnership milestones" ON public.milestones AS PERMISSIVE FOR DELETE TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Members can update partnership milestones" ON public.milestones AS PERMISSIVE FOR UPDATE TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Members can view partnership milestones" ON public.milestones AS PERMISSIVE FOR SELECT TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));

-- net_worth_snapshots
CREATE POLICY "Users can insert own partnership snapshots" ON public.net_worth_snapshots AS PERMISSIVE FOR INSERT TO public WITH CHECK ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Users can view own partnership snapshots" ON public.net_worth_snapshots AS PERMISSIVE FOR SELECT TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));

-- notifications
CREATE POLICY "Users can insert own notifications" ON public.notifications AS PERMISSIVE FOR INSERT TO public WITH CHECK ((user_id = auth.uid()));
...
-- partner_link_requests
CREATE POLICY "Users can create link requests" ON public.partner_link_requests AS PERMISSIVE FOR INSERT TO public WITH CHECK ((requester_user_id = auth.uid()));
CREATE POLICY "Users can update own link requests" ON public.partner_link_requests AS PERMISSIVE FOR UPDATE TO public USING (((target_user_id = auth.uid()) OR (requester_user_id = auth.uid())));
CREATE POLICY "Users can view own link requests" ON public.partner_link_requests AS PERMISSIVE FOR SELECT TO public USING (((requester_user_id = auth.uid()) OR (target_user_id = auth.uid())));

-- partnership_members
CREATE POLICY "Members can view membership" ON public.partnership_members AS PERMISSIVE FOR SELECT TO public USING ((partnership_id IN ( SELECT get_user_partnerships.partnership_id FROM private.get_user_partnerships(( SELECT auth.uid() AS uid)) get_user_partnerships(partnership_id))));
CREATE POLICY "Users can join partnerships" ON public.partnership_members AS PERMISSIVE FOR INSERT TO public WITH CHECK ((user_id = auth.uid()));

-- partnerships
CREATE POLICY "Members can view their partnerships" ON public.partnerships AS PERMISSIVE FOR SELECT TO public USING ((id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Owners can update partnerships" ON public.partnerships AS PERMISSIVE FOR UPDATE TO public USING ((id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE ((partnership_members.user_id = auth.uid()) AND (partnership_members.role = 'owner'::text)))));
CREATE POLICY "Users can create partnerships" ON public.partnerships AS PERMISSIVE FOR INSERT TO public WITH CHECK (true);
...
-- savings_goals
CREATE POLICY "Members can create partnership goals" ON public.savings_goals AS PERMISSIVE FOR INSERT TO public WITH CHECK ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Members can delete partnership goals" ON public.savings_goals AS PERMISSIVE FOR DELETE TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Members can update partnership goals" ON public.savings_goals AS PERMISSIVE FOR UPDATE TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Members can view partnership goals" ON public.savings_goals AS PERMISSIVE FOR SELECT TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));

-- tags
CREATE POLICY "Anyone can insert tags" ON public.tags AS PERMISSIVE FOR INSERT TO public WITH CHECK (true);
CREATE POLICY "Anyone can view tags" ON public.tags AS PERMISSIVE FOR SELECT TO public USING (true);
CREATE POLICY "Authenticated users can update tags" ON public.tags AS PERMISSIVE FOR UPDATE TO public USING ((auth.uid() IS NOT NULL));

-- target_allocations
CREATE POLICY "Members can create target allocations" ON public.target_allocations AS PERMISSIVE FOR INSERT TO public WITH CHECK ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Members can delete target allocations" ON public.target_allocations AS PERMISSIVE FOR DELETE TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Members can update target allocations" ON public.target_allocations AS PERMISSIVE FOR UPDATE TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Members can view target allocations" ON public.target_allocations AS PERMISSIVE FOR SELECT TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));

-- transaction_category_overrides
CREATE POLICY "Users can delete their own overrides" ON public.transaction_category_overrides AS PERMISSIVE FOR DELETE TO public USING (((changed_by = auth.uid()) AND (transaction_id IN ( SELECT t.id FROM (transactions t JOIN accounts a ON ((a.id = t.account_id))) WHERE (a.user_id = auth.uid())))));
CREATE POLICY "Users can manage overrides for their own transactions" ON public.transaction_category_overrides AS PERMISSIVE FOR INSERT TO public WITH CHECK (((changed_by = auth.uid()) AND (transaction_id IN ( SELECT t.id FROM (transactions t JOIN accounts a ON ((a.id = t.account_id))) WHERE (a.user_id = auth.uid())))));
CREATE POLICY "Users can update their own overrides" ON public.transaction_category_overrides AS PERMISSIVE FOR UPDATE TO public USING (((changed_by = auth.uid()) AND (transaction_id IN ( SELECT t.id FROM (transactions t JOIN accounts a ON ((a.id = t.account_id))) WHERE (a.user_id = auth.uid())))));
CREATE POLICY "Users can view overrides for their transactions" ON public.transaction_category_overrides AS PERMISSIVE FOR SELECT TO public USING ((transaction_id IN ( SELECT t.id FROM ((transactions t JOIN accounts a ON ((a.id = t.account_id))) JOIN partnership_members pm ON ((pm.user_id = a.user_id))) WHERE (pm.user_id = auth.uid()))));

-- transaction_notes
CREATE POLICY "Partners can view shared notes" ON public.transaction_notes AS PERMISSIVE FOR SELECT TO public USING (((is_partner_visible = true) AND (user_id IN ( SELECT get_partner_user_ids.user_id FROM private.get_partner_user_ids(( SELECT auth.uid() AS uid)) get_partner_user_ids(user_id)))));
CREATE POLICY "Users can delete their own transaction notes" ON public.transaction_notes AS PERMISSIVE FOR DELETE TO public USING ((user_id = auth.uid()));
CREATE POLICY "Users can insert their own transaction notes" ON public.transaction_notes AS PERMISSIVE FOR INSERT TO public WITH CHECK ((user_id = auth.uid()));
CREATE POLICY "Users can update their own transaction notes" ON public.transaction_notes AS PERMISSIVE FOR UPDATE TO public USING ((user_id = auth.uid()));
CREATE POLICY "Users can view their own transaction notes" ON public.transaction_notes AS PERMISSIVE FOR SELECT TO public USING ((user_id = auth.uid()));

-- transaction_references
CREATE POLICY "Users can delete transaction references" ON public.transaction_references AS PERMISSIVE FOR DELETE TO public USING ((up_transaction_id IN ( SELECT t.up_transaction_id FROM (transactions t JOIN accounts a ON ((t.account_id = a.id))) WHERE (a.user_id = auth.uid()))));
CREATE POLICY "Users can insert transaction references" ON public.transaction_references AS PERMISSIVE FOR INSERT TO public WITH CHECK ((up_transaction_id IN ( SELECT t.up_transaction_id FROM (transactions t JOIN accounts a ON ((t.account_id = a.id))) WHERE (a.user_id = auth.uid()))));
CREATE POLICY "Users can view transaction references" ON public.transaction_references AS PERMISSIVE FOR SELECT TO public USING ((up_transaction_id IN ( SELECT t.up_transaction_id FROM (transactions t JOIN accounts a ON ((t.account_id = a.id))) WHERE (a.user_id = auth.uid()))));

-- transaction_share_overrides
CREATE POLICY "Users can delete their partnership's transaction overrides" ON public.transaction_share_overrides AS PERMISSIVE FOR DELETE TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Users can insert transaction overrides for their partnership" ON public.transaction_share_overrides AS PERMISSIVE FOR INSERT TO public WITH CHECK ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Users can update their partnership's transaction overrides" ON public.transaction_share_overrides AS PERMISSIVE FOR UPDATE TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Users can view their partnership's transaction overrides" ON public.transaction_share_overrides AS PERMISSIVE FOR SELECT TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));

-- transaction_tags
CREATE POLICY "Users can insert transaction tags" ON public.transaction_tags AS PERMISSIVE FOR INSERT TO public WITH CHECK ((transaction_id IN ( SELECT t.id FROM (transactions t JOIN accounts a ON ((a.id = t.account_id))) WHERE (a.user_id = auth.uid()))));
CREATE POLICY "Users can view their transaction tags" ON public.transaction_tags AS PERMISSIVE FOR SELECT TO public USING ((transaction_id IN ( SELECT t.id FROM (transactions t JOIN accounts a ON ((a.id = t.account_id))) WHERE (a.user_id = auth.uid()))));

-- transactions
CREATE POLICY "Partners can view each others transactions" ON public.transactions AS PERMISSIVE FOR SELECT TO public USING ((account_id IN ( SELECT a.id FROM accounts a WHERE (a.user_id IN ( SELECT get_partner_user_ids.user_id FROM private.get_partner_user_ids(( SELECT auth.uid() AS uid)) get_partner_user_ids(user_id))))));
CREATE POLICY "Users can insert own transactions" ON public.transactions AS PERMISSIVE FOR INSERT TO public WITH CHECK ((account_id IN ( SELECT accounts.id FROM accounts WHERE (accounts.user_id = auth.uid()))));
CREATE POLICY "Users can insert transactions" ON public.transactions AS PERMISSIVE FOR INSERT TO public WITH CHECK ((account_id IN ( SELECT accounts.id FROM accounts WHERE (accounts.user_id = auth.uid()))));
CREATE POLICY "Users can update own transactions" ON public.transactions AS PERMISSIVE FOR UPDATE TO public USING ((account_id IN ( SELECT accounts.id FROM accounts WHERE (accounts.user_id = auth.uid()))));
CREATE POLICY "Users can view own transactions" ON public.transactions AS PERMISSIVE FOR SELECT TO public USING ((account_id IN ( SELECT accounts.id FROM accounts WHERE (accounts.user_id = auth.uid()))));

-- up_api_configs
CREATE POLICY "Enable delete for users" ON public.up_api_configs AS PERMISSIVE FOR DELETE TO public USING ((auth.uid() = user_id));
CREATE POLICY "Enable insert for users" ON public.up_api_configs AS PERMISSIVE FOR INSERT TO public WITH CHECK ((auth.uid() = user_id));
CREATE POLICY "Enable read for users" ON public.up_api_configs AS PERMISSIVE FOR SELECT TO public USING ((auth.uid() = user_id));
CREATE POLICY "Enable update for users" ON public.up_api_configs AS PERMISSIVE FOR UPDATE TO public USING ((auth.uid() = user_id));

-- user_budgets
CREATE POLICY "Members can create partnership budgets" ON public.user_budgets AS PERMISSIVE FOR INSERT TO public WITH CHECK ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Members can delete partnership budgets" ON public.user_budgets AS PERMISSIVE FOR DELETE TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Members can update partnership budgets" ON public.user_budgets AS PERMISSIVE FOR UPDATE TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Members can view partnership budgets" ON public.user_budgets AS PERMISSIVE FOR SELECT TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));

-- user_dashboard_charts
CREATE POLICY "Users can delete their own charts" ON public.user_dashboard_charts AS PERMISSIVE FOR DELETE TO public USING ((auth.uid() = user_id));
CREATE POLICY "Users can insert their own charts" ON public.user_dashboard_charts AS PERMISSIVE FOR INSERT TO public WITH CHECK ((auth.uid() = user_id));
CREATE POLICY "Users can update their own charts" ON public.user_dashboard_charts AS PERMISSIVE FOR UPDATE TO public USING ((auth.uid() = user_id));
CREATE POLICY "Users can view their own charts" ON public.user_dashboard_charts AS PERMISSIVE FOR SELECT TO public USING ((auth.uid() = user_id));

-- watchlist_items
CREATE POLICY "Members can create watchlist items" ON public.watchlist_items AS PERMISSIVE FOR INSERT TO public WITH CHECK ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Members can delete watchlist items" ON public.watchlist_items AS PERMISSIVE FOR DELETE TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Members can update watchlist items" ON public.watchlist_items AS PERMISSIVE FOR UPDATE TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
CREATE POLICY "Members can view watchlist items" ON public.watchlist_items AS PERMISSIVE FOR SELECT TO public USING ((partnership_id IN ( SELECT partnership_members.partnership_id FROM partnership_members WHERE (partnership_members.user_id = auth.uid()))));
